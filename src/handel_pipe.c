...
static void	setup_child_process(t_node *cmd, int prev_pipe, int pipefds[2],
...
static void	handle_child_process(t_node *cmd, int prev_pipe, int pipefds[2],
		int i, int num_commands, t_env *env)
{
	setup_child_process(cmd, prev_pipe, pipefds, i, num_commands);
	execute_command(cmd, env);
}

static void	handle_parent_process(int *prev_pipe, int pipefds[2], pid_t pid,
		pid_t *last_pid)
{
	*last_pid = pid;
	close(pipefds[1]);
